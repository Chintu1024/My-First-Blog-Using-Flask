...
def dashboard():
    ...
    if request.method=='POST':
        ...
        if (username == params['admin_user'] and userpwd == params['admin_password']):
...
def edit(sno):
    if 'user' in session and session['user'] == params['admin_user']:
        if request.method=='POST':
            ...
            if sno == '0':
                post = Posts(title=box_title,slug=box_slug,content=box_content,date= datetime.now(),tagline=box_tline,img_file=box_img_file)
                db.session.add(post)
                db.session.commit()
            else:
